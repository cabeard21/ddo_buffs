...
class BuffBar(QWidget):
    def __init__(self):
        ...
        handler = logging.FileHandler(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'buff_bar.log'))
    ...
    def initUI(self):
        ...
        self.layout.setContentsMargins(10, 0, 10, 0)  # (left, top, right, bottom)
    ...
    def update(self, buff, remaining):
        ...
        if buff in self.bars:
            ...
            self.logger.debug(f'Updated buff bar for {buff} with remaining time {remaining}')
        else:
            ...
            self.logger.debug(f'Created new buff bar for {buff} with remaining time {remaining}')
...
class TimerWidget(QWidget):
    ...
    def initUI(self, buff):
        ...
        self.iconLabel.setPixmap(QIcon(str(BASE / f'buffs/{buff}.png')).pixmap(22, 22))
